...
2. Read the brief: `brief.task` (description, steerDetails, nonGoals, references), `brief.guardrails`, `brief.criteria` (each with its `verify` method), `brief.approvedPlan` (the plan and its validation plan), `brief.verification` (what the verifier ran on the submitted commit, with the failing commands), `brief.openFindings`, `brief.humanDecisions`, `brief.humanNotes` and `brief.conventionFiles`
