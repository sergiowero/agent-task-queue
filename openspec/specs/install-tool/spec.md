...
The install tool page SHALL offer an "Execute" option that runs the AgentQ install script via the backend API and streams the output in real-time.
...
The install tool page SHALL display the `agentq-workflow` skill file content with a copy-to-clipboard button as a fallback.
