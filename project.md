...
The CLI should be built as a practical command layer for coding agents, with simple commands for discovery, task claiming, progress updates, and handoff to review.

1. Task claiming
   - Agent actions: claim a task.
   - What the agent sees: the claimed task's details and confirmation that the task was claimed successfully
   - Overall design: a command such as `atq claim` which allow passing the agent details and get the task details in a structured format. The CLI should prevent claiming tasks that are already claimed or in an invalid state for the agent's role.

2. Task progress updates handoff
   - Agent actions: submit a plan, submit code, submit review, and submit merge. All thsi cation usually adds a conversation entry with the agent's context and progress notes.
   - What the agent sees: confirmation of the action taken, any feedback from the system, and the updated task status.
   - Overall design: commands like `atq submit-plan`, `atq submit-code`, `atq submit-review`, and `atq submit-merge` that accept relevant parameters and provide structured output.
   - Review-specific behavior: a user action can move a task to **Code review requested**; once a reviewer agent claims that task, the system moves it to **Reviewing** and the CLI should expose that state in its output so the agent can see it is actively being reviewed.
   Note: Agents lost ownnership of the task after submitting a plan, code, or review. The system should automatically update the task status and notify the next responsible agent or user.

3. Task creation
   - Agent actions: create a new task with title, description, acceptance criteria, priority, recommended branch, and flags for planning and review requirements. Agents must refine all task details before submission to ensure clarity and completeness.
   - What the agent sees: confirmation of task creation with the new task's ID and initial status.
   - Overall design: a command like `atq create-task` that accepts parameters for all required fields and outputs the created task in a structured format.

### Web Server

The web server will run locally and support interaction with the web portal. It will expose a REST API for managing the system entities and task workflow.
...
| contexts | string[] | List of context provided by codign agents each time submit changes to teh task. |
...
Most likely agents entities will be changing a lot as session ids changes on evey new request.
...
- **Plan Changes Requested** – Entered from **Waiting Plan Review** when reviewers request modifications to the plan. The agent must revise and resubmit. (Returns to **Planning** or flor to **Canceled**)
...
- **Mergeing** – Entered from **Approved** when an agent is actively merging the implementation into the target `mergeBranch`. (Flows to **Merged**)
...
- requeredPlan: is immutaable and cannot be changed after task creation. It determines whether the task starts in the Planning or Ready state.
...
- **Submit merge** – Moves a task from **Mergeing** to **Merged**.
- **Claim task** – An agent cant take a taks in diffrerent conditions like:
  ...
  - From **Approved** to **Mergeing**, if the task is ready to be merged into the target branch. Agent commit and merge to the target branch and update the task status to Merged.
...
- **Agent Claims Task**: An agent claims the task, moving it to the **Planning** state. The agent develops an implementation plan and submits it (adding the plan to teh conversation), transitioning the task to **Waiting Plan Review**.
...
Agents can fail or crachs, leaving tasks in a state where they cannot progress. The system should provide mechanisms for users to unblock these tasks and allow other agents to take over.
