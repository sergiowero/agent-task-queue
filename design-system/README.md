...
- Map workflow state to tone through `TASK_STATUS` in `lib/status.ts`, never ad hoc. Labels and board columns come from the shared status catalog (`@agentq/shared/catalog`); `lib/status.ts` only adds tone and icon, typed per status so a new status fails typecheck until it has a look. `accent` = planning, `info` = ready and coding, `warning` = anything waiting on review (plus merging), `primary` = PR open, `success` = approved and complete, `danger` = changes requested, canceled and needs human, `neutral` = requested and not started.
- Board columns follow the same map: Pending is neutral, In progress is info, Needs you is warning and Done is success.
