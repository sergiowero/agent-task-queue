...
describe("AgentQ MCP claims and blockers", () => {
  ...
  it("the reviewer's session never sees the coder's context: claim, brief, get_task, list_tasks, comments, resource", async () => {
    ...
    const coder = await connect();
    const reviewer = await connect();
    parse(await call(coder, "claim_task", { ...agent, role: "implementer", sessionId: "ic1", projectId }));
    ...
    const claimed = parse(await call(reviewer, "claim_task", { ...agent, role: "reviewer", sessionId: "ir1", projectId }));
