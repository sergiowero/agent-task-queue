...
Every tool returns JSON both as a text content block and as `structuredContent`. Successful results carry `success: true`; failures return `{ "success": false, "error": "..." }` with `isError: true`, using the workflow's messages (`Task not found.`, `Task must be in Planning status.`, ...). Arguments that fail validation (a missing field, an unknown role or an empty `roles` list) come back as an `isError` result whose text starts with `MCP error -32602: Input validation error`.

Clients prefix the names: in Claude Code `claim_task` is `mcp__agentq__claim_task`, in OpenCode `agentq_claim_task`.

| Tool            | Description                                                                                                                                                                                                                |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `claim_task`    | Atomically claim the highest-priority task one of your roles works and move it to its in-progress status (`refining`, `planning`, `plan_reviewing`, `coding`, `verifying`, `reviewing` or `merging`); `agent.role` in the result is the role the claim acts as. Returns `{ success, task (without conversation/history), brief, phaseSkill, agent, claimToken, skillsVersion, skills }` (for a plan critique, verification or review, `task` is the independent brief's task summary and `brief` the independent brief; see [policy.md](policy.md#independent-checks)), `{ success: false, reason: "no_tasks_available" }` or, for agents whose skills are older than the server supports, `{ success: false, reason: "skills_outdated" }`. |
...
| `submit_pr`     | Record the pull request opened for a task in `merging` (`prUrl`, base and head branches, pushed commit, authors); moves it to `pr_open` and releases it. The task completes when the PR is merged on GitHub (the server follows it with `gh`), goes to `needs_human` if it is closed, and a person can mark it merged from the portal. `get_task_brief` returns the PR body to use as `pr.body`. |
| `submit_merge`  | Deprecated alias of `submit_pr` for older skills: same inputs, the PR URL may come in `message`. |
...
| `get_task_brief` | What an agent needs to continue a task: approved plan and validation, criteria, open findings, latest handoff per phase, project commands/conventions/guardrails, round, and what people said since the last submission. Its size does not grow with rounds. For a task in plan critique, verification or review it is the **independent brief** (`independent: true`): the task, the criteria and how each is checked, the plan, the findings to verify and what people decided, without the author's conversation, handoffs, messages or evidence. |
| `get_skill`     | The current text and version of an AgentQ skill, as this server ships it.                                                                                                                                                  |
| `get_task`      | Fetch a task by ID with its project, its review `findings`, its `evidence` and its `handoffs`. To the session holding the task in plan critique, verification or review it returns `{ independent: true, task, brief, message }` instead (so do `list_tasks`, `post_comment` and the task resource).                                                                                                                                           |
| `submit_plan_review` | Critique a plan in `plan_reviewing`: `verdict` and findings (`P<round>-<n>`); routes the task (see [policy.md](policy.md)). |
| `create_subtask` | Planner only, holding the parent in `planning`: create a subtask (held until the plan is approved; `blockedBy` for order). |
| `submit_refinement` | Refiner only, on a task in `refining`: make the draft ready (criteria, type, risk, non-goals, requiresPlan, open questions). |
| `heartbeat`     | Extend the lease of a claim made by a hand-opened session (any call from the session already does).                                                                                                                      |
| `submit_verification` | With the `verify` role: report a task in `verifying` (`passed`, `evidence`, `tampering`). The built-in verifier calls the same workflow directly.                                                                     |
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `claim_task`    | `toolName`, `version`, `model`, `sessionId` | `roles` (one or more of `refine`, `plan`, `plan_review`, `code`, `verify`, `review`, `pr`; default all but `verify`), `host`, `projectId`, `skillsVersion`, `context`                                                                     |
...
| `submit_pr`     | `taskId`, `mergeBranch`, `commit`, `authors`, `context`                                                                   | `prUrl`, `prNumber`, `headBranch`, `message`, `worktree`, `author`, `claimToken`, `agentId`                          |
| `submit_merge`  | as `submit_pr`                                                                                                            | as `submit_pr`                                                                                                         |
| `report_blocker` | `taskId`, `reason`, `question`                                                                                           | `context`, `author`, `claimToken`, `agentId`                                                                           |
| `get_task`      | `taskId`                                                                                                                  |                                                                                                                        |
| `get_task_brief` | `taskId`                                                                                                                 |                                                                                                                        |
| `get_skill`     | `name`                                                                                                                    |                                                                                                                        |
