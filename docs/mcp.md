...
| `submit_code`   | Submit code for a task in `coding`; stores the worktree path and releases it. Under autonomy L1+ it goes straight to `code_review_requested` (an AI reviewer); under L0 to `waiting_code_review`.                     |
...
| `get_task`      | Fetch a task by ID with its project and its review `findings`.                                                                                                                                                           |
...
| `create_task`   | Create a task. `guardrails` and `acceptanceCriteria` are arrays; entries are trimmed and empty ones dropped. Without `mergeBranch` the task targets the project's default branch (detected from `origin/HEAD`). Logs `task_created`. |
...
| `submit_plan`   | `taskId`, `message`, `context`                                                                                            | `author`, `claimToken`, `agentId`                                                                                      |
| `submit_code`   | `taskId`, `message`, `worktree`, `context`                                                                                | `author`, `claimToken`, `agentId`                                                                                      |
