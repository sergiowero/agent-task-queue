...
| `get_task_brief` | What an agent needs to continue a task: approved plan and validation, criteria, open findings, latest handoff per phase, project commands/guardrails, round, and what people said since the last submission. Its size does not grow with rounds. For a task in plan critique, verification or review it is the **independent brief** (`independent: true`): the task, the criteria and how each is checked, the plan, the findings to verify and what people decided, without the author's conversation, handoffs, messages or evidence. |
